...
const BUSINESSPLAN_FIELDS = [
  ...
  { key: 'phase_ref', label: 'Phase (optional)', type: 'applookup/select', targetEntity: 'phasen', targetAppId: 'PHASEN', displayField: 'phasen_name' },
  { key: 'bp_titel', label: 'Titel des Businessplans', type: 'string/text' },
  { key: 'executive_summary', label: 'Executive Summary', type: 'string/textarea' },
  { key: 'unternehmenskonzept', label: 'Unternehmenskonzept', type: 'string/textarea' },
  { key: 'marktanalyse', label: 'Marktanalyse', type: 'string/textarea' },
  { key: 'wettbewerbsanalyse', label: 'Wettbewerbsanalyse', type: 'string/textarea' },
  { key: 'marketingstrategie', label: 'Marketingstrategie', type: 'string/textarea' },
  { key: 'betriebsplan', label: 'Betriebsplan', type: 'string/textarea' },
  { key: 'team', label: 'Team / Gründerperson', type: 'string/textarea' },
  { key: 'rechtsform_bp', label: 'Rechtsform', type: 'string/text' },
  { key: 'swot_analyse', label: 'SWOT-Analyse', type: 'string/textarea' },
  { key: 'bp_datei', label: 'Businessplan hochladen (optional)', type: 'file' },
];
const FINANZPLAN_FIELDS = [
  { key: 'gruender_ref', label: 'Gründer', type: 'applookup/select', targetEntity: 'gruenderprofil', targetAppId: 'GRUENDERPROFIL', displayField: 'vorname' },
  { key: 'businessplan_ref', label: 'Businessplan', type: 'applookup/select', targetEntity: 'businessplan', targetAppId: 'BUSINESSPLAN', displayField: 'bp_titel' },
  { key: 'startkapital', label: 'Startkapitalbedarf (EUR)', type: 'number' },
  { key: 'eigenkapital', label: 'Eigenkapital (EUR)', type: 'number' },
  { key: 'fremdkapital', label: 'Fremdkapital (EUR)', type: 'number' },
  { key: 'kapitalverwendung', label: 'Verwendung des Kapitals', type: 'string/textarea' },
  { key: 'umsatz_jahr1', label: 'Umsatzplanung Jahr 1 (EUR)', type: 'number' },
  { key: 'umsatz_jahr2', label: 'Umsatzplanung Jahr 2 (EUR)', type: 'number' },
  { key: 'umsatz_jahr3', label: 'Umsatzplanung Jahr 3 (EUR)', type: 'number' },
  { key: 'kosten_fix', label: 'Fixkosten pro Monat (EUR)', type: 'number' },
  { key: 'kosten_var', label: 'Variable Kosten pro Monat (EUR)', type: 'number' },
  { key: 'kostenbeschreibung', label: 'Kostenbeschreibung', type: 'string/textarea' },
  { key: 'liquiditaet_start', label: 'Startliquidität (EUR)', type: 'number' },
  { key: 'liquiditaetsplanung', label: 'Liquiditätsplanung', type: 'string/textarea' },
  { key: 'break_even', label: 'Break-Even-Umsatz (EUR)', type: 'number' },
  { key: 'break_even_beschreibung', label: 'Break-Even-Beschreibung', type: 'string/textarea' },
  { key: 'rentabilitaet', label: 'Rentabilitätsvorschau Jahr 1 (EUR)', type: 'number' },
  { key: 'fp_datei', label: 'Finanzplan hochladen (optional)', type: 'file' },
];
const MEILENSTEINE_FIELDS = [
  ...
  { key: 'ms_status', label: 'Status', type: 'lookup/select', options: [{ key: 'erreicht', label: 'Erreicht' }, { key: 'nicht_erreicht', label: 'Nicht erreicht' }, { key: 'offen', label: 'Offen' }, { key: 'in_arbeit', label: 'In Arbeit' }] },
  ...
  { key: 'ms_dokument', label: 'Dokument hochladen (optional)', type: 'file' },
];
const AUFGABEN_FIELDS = [
  ...
  { key: 'aufgabe_dokument', label: 'Dokument hochladen (optional)', type: 'file' },
];
const GRUENDERPROFIL_FIELDS = [
  { key: 'gruender_dokument', label: 'Dokument hochladen (optional)', type: 'file' },
...
const PHASEN_FIELDS = [
  ...
  { key: 'phase_status', label: 'Status der Phase', type: 'lookup/radio', options: [{ key: 'offen', label: 'Offen' }, { key: 'in_arbeit', label: 'In Arbeit' }, { key: 'abgeschlossen', label: 'Abgeschlossen' }] },
...
const ROADMAPCOCKPIT_FIELDS = [
  ...
  { key: 'phase_ampel_aktuell', label: 'Aktueller Phasenstatus (Ampel)', type: 'lookup/radio', options: [{ key: 'gruen', label: 'Grün – Auf Kurs' }, { key: 'gelb', label: 'Gelb – Leichte Verzögerung / offene Punkte' }, { key: 'rot', label: 'Rot – Kritische Probleme / Stopp-Risiko' }] },
...
const ENTITY_TABS = [
  ...
  { key: 'businessplan', label: 'Businessplan', pascal: 'Businessplan' },
  { key: 'finanzplan', label: 'Finanzplan', pascal: 'Finanzplan' },
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('vorlagen_&_tools');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'vorlagen_&_tools': new Set(),
    'businessplan': new Set(),
    'finanzplan': new Set(),
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'businessplan': return (data as any).businessplan as Businessplan[] ?? [];
      case 'finanzplan': return (data as any).finanzplan as Finanzplan[] ?? [];
  ...
  const getLookupLists = useCallback((entity: EntityKey) => {
    ...
    switch (entity) {
      ...
      case 'finanzplan':
        lists.gruenderprofilList = (data as any).gruenderprofil ?? [];
        lists.businessplanList = (data as any).businessplan ?? [];
        break;
      case 'meilensteine':
        lists.gruenderprofilList = (data as any).gruenderprofil ?? [];
        lists.phasenList = (data as any).phasen ?? [];
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'businessplan' && fieldKey === 'gruender_ref') {
      const match = (lists.gruenderprofilList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.vorname ?? '—';
    }
    if (entity === 'businessplan' && fieldKey === 'phase_ref') {
      const match = (lists.phasenList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.phasen_name ?? '—';
    }
    if (entity === 'finanzplan' && fieldKey === 'gruender_ref') {
      const match = (lists.gruenderprofilList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.vorname ?? '—';
    }
    if (entity === 'finanzplan' && fieldKey === 'businessplan_ref') {
      const match = (lists.businessplanList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.bp_titel ?? '—';
    }
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'businessplan': return BUSINESSPLAN_FIELDS;
      case 'finanzplan': return FINANZPLAN_FIELDS;
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'businessplan': return {
        create: (fields: any) => LivingAppsService.createBusinessplanEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateBusinessplanEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteBusinessplanEntry(id),
      };
      case 'finanzplan': return {
        create: (fields: any) => LivingAppsService.createFinanzplanEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateFinanzplanEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteFinanzplanEntry(id),
      };
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'businessplan' || dialogState?.entity === 'businessplan') && (
        <BusinessplanDialog
          open={createEntity === 'businessplan' || dialogState?.entity === 'businessplan'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'businessplan' ? handleUpdate : (fields: any) => handleCreate('businessplan', fields)}
          defaultValues={dialogState?.entity === 'businessplan' ? dialogState.record?.fields : undefined}
      ...
      {(createEntity === 'finanzplan' || dialogState?.entity === 'finanzplan') && (
        <FinanzplanDialog
          open={createEntity === 'finanzplan' || dialogState?.entity === 'finanzplan'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'finanzplan' ? handleUpdate : (fields: any) => handleCreate('finanzplan', fields)}
          defaultValues={dialogState?.entity === 'finanzplan' ? dialogState.record?.fields : undefined}
          gruenderprofilList={(data as any).gruenderprofil ?? []}
          businessplanList={(data as any).businessplan ?? []}
      ...
      {viewState?.entity === 'businessplan' && (
        <BusinessplanViewDialog
          open={viewState?.entity === 'businessplan'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'businessplan', record: r }); }}
          gruenderprofilList={(data as any).gruenderprofil ?? []}
          phasenList={(data as any).phasen ?? []}
        />
      )}
      {viewState?.entity === 'finanzplan' && (
        <FinanzplanViewDialog
          open={viewState?.entity === 'finanzplan'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'finanzplan', record: r }); }}
          gruenderprofilList={(data as any).gruenderprofil ?? []}
          businessplanList={(data as any).businessplan ?? []}
        />
      )}
